...
@onready var mushroom_mimic: AnimatedSprite2D = $MushroomMimic
...
func _ready() -> void:
	...
	#mushroom_mimic.play("unactivated_down")
...
func _update_facing(dir: Vector2) -> void:
	if abs(dir.x) > abs(dir.y):
		if dir.x > 0:
			...
			facing = "right"
			mushroom_mimic.flip_h = true
		else:
			# Moving left → use default orientation (no flip)
			facing = "left"
			mushroom_mimic.flip_h = false
	else:
...
func _physics_process(delta: float) -> void:
	...
	if dist > attack_range:
		...
		if mushroom_mimic.animation != anim and not is_attacking:
			mushroom_mimic.play(anim)
		...
		if is_attacking:
			...
			if mushroom_mimic.animation != idle_anim:
				mushroom_mimic.play(idle_anim)
		else:
			_start_attack()
...
func _on_detected_area_body_entered(body: Node2D) -> void:
	if body is Player:
		...
		if not is_activated and not is_activating:
			is_activating = true
			mushroom_mimic.play("activate_down")
...
func _on_rune_sentry_animation_finished() -> void:
	...
	elif mushroom_mimic.animation == "activate_down":
		is_activated = true
		is_activating = false
		mushroom_mimic.play("idle_down")
